...
class TestCMakeListsGeneratorTest():
    ...
    def test_generate_cmake(self):
        # Fake file path
        file_location = Path(".").absolute() / "test_files/expected_test_cmakelists.txt"
        ...
        expected_cmake_lists = open(Path(".").absolute() / "test_files/expected_test_cmakelists.txt").read()
